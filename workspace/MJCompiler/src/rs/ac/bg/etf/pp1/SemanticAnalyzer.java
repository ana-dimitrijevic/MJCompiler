...
public class SemanticAnalyzer extends VisitorAdaptor {
	...
	public void visit(FuncCallStart funcCallStart) {
		...
		if (!findResult.equals(Tab.noObj)) {
			...
			if (Obj.Meth == findResult.getKind()) {
				...
				report_error("Greska: " + funcCallStart.getDesignator().obj.getName() + " nije metoda ",
						funcCallStart);
				isAFunction = false;
				return;
			}
	...
	public void visit(ProcCallStart procCallStart) {
		...
		if (!findResult.equals(Tab.noObj)) {
			...
			if (findResult.getKind() != Obj.Meth) {
				...
				report_error("Greska: " + procCallStart.getDesignator().obj.getName() + " nije metoda ",
					procCallStart);
				// isAProcCall = false;
				isAFunction = false;
				return;
	...
	public void visit(ProcCall procCall) {
		...
		if (method.getLevel() != currCallArgTypes.size()) {
			report_error("Greska: Prosledjeno: " + currCallArgTypes.size() + "; Ocekivano: "
					+  method.getLevel() + " argumenata" , procCall);
			//errorDetected = true;
			return;
		}
		...
		for (Obj obj : currCallFormalArgTypes) {
			...
			if (!foundType.assignableTo(obj.getType()) 
				|| !foundType.compatibleWith(obj.getType())) {
				report_error("Greska: razlikuju se tip ocekivanog i stvarnog argumenta na poziciji " + (count + 1), procCall);
				//errorDetected = true;
				return;
			}
			count++;
	...
	public void visit(DesignatorName designatorName) {
		...
		//ne samo to nego mora i da bude deo designator statementa
		if (designatorName.obj.getFpPos()!=-1 && designatorName.obj.getKind() != Obj.Meth && !designatorName.getName().equals("null"))
		report_info("Detektovana upotreba formalnog argumenta " + designatorName.getName(), designatorName);
		
		//report_info(designatorName.getName() + " fp pos= " + designatorName.obj.getFpPos(), designatorName);
